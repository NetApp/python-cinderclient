...
class Share(base.Resource):
    ...
    def deny(self, id):
        """
        Deny access from IP to a volume (shares type only).
        """
...
class ShareManager(base.ManagerWithFind):
    ...
    def get(self, share_id):
        ...
        :param volume_id: The ID of the share to delete.
        :rtype: :class:`Share`
        """
    ...
    def allow(self, volume, access_type, access):
        """
        Allow access from IP to a volume (shares type only).

        :param volume: The :class:`Share` to delete.
        :param access_type: string that represents access type ('ip','domain')
        :param access: string that represents access ('127.0.0.1')
        """
        return self._action('os-access_allow', volume,
                {'access_type': access_type,
                 'access_to': access})

    def deny(self, volume, id):
        """
        Deny access from IP to a volume (shares type only).
        ...
        """
        return self._action('os-access_deny', volume, {'access_id': id})

    def access_list(self, volume):
        """
        Get access list to the volume
        """
        access_list = self._action("os-access_list", volume)[1]["access_list"]
        if access_list:
